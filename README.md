Lab - Postgres Operator
========================

This workshop provides an introduction to deploying a Postgres cluster using the Postgres operator. Use of the operator is from the perspective of a developer.
...
When the URL for the workshop environment is accessed, a workshop session will be created on demand. This will include a project for the session, into which the Postgres operator will have been pre-installed.
...
Only delete these last set of resources if the Postgres operator is not being used elsewhere in the cluster. Ideally this workshop environment should only be deployed in an expendable cluster, and not one which is shared for other work.
